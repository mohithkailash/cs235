...
class NaiveBAlg():
    ...
    def fit(self, X_train, y_train):
            ...
            for i in range(y_train.shape[0]):
                ...
                if y_train[i] == 1:
                    if first_one == True:
                        ...
                        class_data_dic[1] = np.append(
                            class_data_dic[1], X_temp, axis=1)
                elif y_train[i] == 0:
                    if first_zero == True:
                        class_data_dic[0] = X_temp
                        first_zero = False
                    else:
                        class_data_dic[0] = np.append(
                            class_data_dic[0], X_temp, axis=1)
        ...
        self.class_data_dic = generate_data(
            self.class_data_dic, self.X_train, self.y_train)
    ...
    def predict(self, X_test):
        def posterior(X, X_train_class, mean_, std_):
            ...
            product = product * \
                (X_train_class.shape[0] / self.X_train.shape[0])
            return product

        p_1 = posterior(
            X_test, self.class_data_dic[1], self.mean_1, self.std_1)
        p_0 = posterior(
            X_test, self.class_data_dic[0], self.mean_0, self.std_0)
        return 1 * (p_1 > p_0)
